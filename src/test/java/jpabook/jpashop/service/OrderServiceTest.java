...
public class OrderServiceTest {
    ...
    public void 주문취소() throws Exception {
        ...
        //then
        Order getOrder = orderRepository.findOne(orderId);
        assertEquals("주문 취소상태는 CANCEL", OrderStatus.CANCEL, getOrder.getStatus());
        assertEquals("주문이 취소된 상품은 재고가 증가해야된다", 10, item.getStockQuantity());
    ...
    private Member createMember() {
        ...
        member.setName("회원1");
        member.setAddress(new Address("seoul", "dangsan", "111"));
        em.persist(member);
        return member;
    }
}
